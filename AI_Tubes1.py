...
def hillClimbing():
	...
	#mulai hill climbing
	while step < 10000 and nKonflikNow > 0:
		...
		for matkul in listKonflikNow:
			#majuin terus sebanyak (nDomain - 1) kali
			for i in range(1, matkul.nDomain):
				step += 1
			...
			else: #matkul ini di geser ke semua domain, tetep gak nemu lebih baik
				if matkul == listKonflikNow[len(listKonflikNow) - 1]:
					lokalMaks = True #kalo ini matkul terakhir, berarti emang gak ada yang lebih baik lagi
				else: #ini bukan matkul terakhir, coba matkul lainnya di geser-geser
					step += 1
					matkul.idxPlus() #balikin dulu ke posisi awal
				nKonflikNow = countConflicts() #count lagi supaya listKonflik juga ke posisi awal
